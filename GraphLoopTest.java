...
//    如果此点没有尚未遍历的邻接点，则将此点从stack中弹出。
    ...
    private Map<String, Boolean> status1 = new HashMap<String, Boolean>();
    public void DFSSearch(String startPoint) {
        stack.push(startPoint);
        status1.put(startPoint, true);
        dfsLoop();
    }
    ...
    private void dfsLoop() {
        ...
        System.out.println(stackTopPoint);
        //  2) 找出与此点邻接的且尚未遍历的点，进行标记，然后全部放入queue中。
        List<String> neighborPoints = graph.get(stackTopPoint);
        for (String point : neighborPoints) {
            if (!status1.getOrDefault(point, false)) { //未被遍历
                stack.push(point);
                status1.put(point, true);
                dfsLoop();
            }
        }
